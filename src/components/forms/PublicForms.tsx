...
export function AuthForm({ mode }: { mode: "login" | "register" | "forgot" }) {
    ...
    try {
      if (mode === "register") {
        ...
        if (error) {
          ...
          // If user is returned and a session exists, create profile row
          const user = (data as any)?.user;
          if (user && user.id) {
            // Insert profile; this should pass RLS for the authenticated user
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4" noValidate>
      {mode === "register" ? (
        <FormInput id="a-name" name="name" label="Name" required error={errors.name} />
      ) : null}

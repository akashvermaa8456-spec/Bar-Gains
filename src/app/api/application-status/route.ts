...
async function hasByProfileOrEmail(
  ...
  for (const [key, value] of Object.entries(filters)) {
    profileQuery = profileQuery.eq(key, value);
  }

  profileQuery = profileQuery.eq("profile_id", userId).limit(1);
  ...
  for (const [key, value] of Object.entries(filters)) {
    emailQuery = emailQuery.eq(key, value);
  }

  emailQuery = emailQuery.eq("email", email).limit(1);

  const { data: byEmail, error: emailError } = await emailQuery;

  if (emailError) throw emailError;

  return {
    applied: Boolean(byEmail?.length),
    matchedBy: byEmail?.length ? "email" : null,
  };
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (authError || !user) {
      console.error(
          "Application status auth failed:",
          authError?.message,
      );
    ...
    if (
        !slug ||
        !["course", "internship", "project"].includes(type ?? "")
    ) {
      return NextResponse.json(
          { error: "Invalid type or slug" },
          { status: 400 },
      );
    }
